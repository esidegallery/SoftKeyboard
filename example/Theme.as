...
//			};
        ...
        private function _key(k:Key):void
        {
            ...
            if (k.charCode == CharCode.NUM_LOCK)
                k.icon = Theme.getImage(SoftKeyboardIconBackspace);
            if (k.charCode == CharCode.BACKSPACE)
                k.icon = Theme.getImage(SoftKeyboardIconBackspace);
            if (k.charCode == CharCode.RETURN)
                k.icon = Theme.getImage(SoftKeyboardIconEnter);
            if (k.charCode == CharCode.TAB)
                k.icon = Theme.getImage(SoftKeyboardIconTab);
...
//			}
         ...
         */
        public static function getScale9Image(source:Class, staticWidth:Number = 25, staticHeight:Number = 25):Scale9Image
        {

            var texture:Texture = Theme.getTexture(source);

            var rect:Rectangle = new Rectangle(staticWidth, staticHeight, texture.width - 2 * staticWidth, texture.height - 2 * staticHeight)
