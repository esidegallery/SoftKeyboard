...
package cc.cote.feathers.softkeyboard
{
    ...
    public class SoftKeyboard extends FeathersControl
    {
        ...
        private function _onShowVariants(e:KeyEvent):void
        {
            ...
            if (_callout != null && _callout.stage != null)
            {
                return;
            }
            ...
            var direction:String = RelativePosition.TOP;
            if (key.height * 1.5 > globalPos.y)
            {
            ...
            try
            {
                _callout = Callout.show(key.variantsContainer, key, new <String>[direction], true);
            }
            catch (e:TypeError)
            {
...
//				throw(error);
            ...
            _callout.height = key.height + _callout.paddingBottom + _callout.paddingTop + _callout.topArrowSkin.height + _callout.topArrowGap;
            _callout.width = (key.width * key.variants.length) + (key.variants.length - 1) * 5 + _callout.paddingLeft + _callout.paddingRight;
        ...
        private function _buildConfiguration(layout:Layout):Sprite
        {
            ...
            {
                throw new Error('SoftKeyboard error: no rows have been defined in the specified layout.')
            }
        ...
        private function _onKeyUp(e:KeyEvent):void
        {
            ...
            if (e.charCode == CharCode.CAPS_LOCK)
            {
				capsLock = !capsLock;
			}    
        ...
        private function _getWidestRow(keyboard:Sprite):KeyRow
        {
            ...
            {
                var row:KeyRow = keyboard.getChildAt(i) as KeyRow;
                if (row.relativeWidth > widest.relativeWidth)
                    widest = row;
            }
